...
#include <QMessageBox>
deletewindow::deletewindow(hash_table* hasht,QWidget *parent)
    : QDialog(parent)
...
void deletewindow::on_pushButton_clicked()
{
    ...
    if(p.control_pole()==1 && firstDotPos != -1 && secondDotPos != -1 )
    {
        ...
        if(t==1)
        {
            emit elementDeleted(p);

            QMessageBox::information(this,"Window", "Удалилось успешно ");
        }
        else
            QMessageBox::warning(this,"Window", "Такого элемента нет ");
    }
